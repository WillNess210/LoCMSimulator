...
#include <sstream>
#include <vector>
//the processes u need to launch both exes and send them inputs for the first turn then read outputs and simu
using namespace std;
class Card {
public:
	...
	string getAbilities() {
		string abilities = "";
		abilities += "-B"[breakthrough];// that's neat fix the rest
		abilities += "-C"[charge];
		abilities += "-D"[drain];
		abilities += "-G"[guard];
		abilities += "-L"[lethal];
...
enum CardType
{
	creature=0, itemGreen=1, itemRed, itemBlue
};
struct CardInfo {
	int cardNumber;
	string name;
...
struct AI
{
	class Process {} process; // placeholder
	string SendInputsAndReceiveOutputs()
	{
	...
	void play() {
		...
		if (turn < 30) { // DRAFT PHASE
			...
			for (int i = 0; i < 2; i++) {
				...
				string action = AIs[i].SendInputsAndReceiveOutputs();
				stringstream ss;
				ss << action;
				string sChoice = "";
				int choice = -1;
